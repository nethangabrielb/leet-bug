    ...
    <form onSubmit={handleSubmit} className="space-y-5">
      <div>
        ...
        <select value={formData.problemId} onChange={(e) => setFormData({ ...formData, problemId: e.target.value })} required
          className="w-full rounded-lg border border-white/[0.08] bg-white/[0.03] px-3 py-2.5 text-sm text-white outline-none transition-colors focus:border-violet-500/50">
        ...
        <div className="flex gap-2">
          {(["YES", "NO", "PARTIAL"] as const).map((opt) => (
            <button key={opt} type="button" onClick={() => setFormData({ ...formData, solved: opt })}
              className={`flex-1 rounded-lg border px-3 py-2 text-sm font-medium transition-all ${formData.solved === opt ? "border-violet-500/40 bg-violet-500/15 text-violet-300" : "border-white/[0.08] bg-white/[0.03] text-white/50 hover:bg-white/[0.06]"}`}>
      ...
      <div>
        ...
        <textarea value={formData.trippedUp} onChange={(e) => setFormData({ ...formData, trippedUp: e.target.value })} placeholder='One sentence — or "Nothing, clean solve"' rows={2}
          className="w-full resize-none rounded-lg border border-white/[0.08] bg-white/[0.03] px-3 py-2.5 text-sm text-white outline-none transition-colors focus:border-violet-500/50" />
      </div>
      ...
      <div className="flex gap-3 pt-2">
        ...
        <button type="submit" disabled={!formData.problemId || loading}
          className="flex-1 rounded-lg bg-gradient-to-r from-violet-500 to-indigo-600 px-4 py-2.5 text-sm font-semibold text-white transition-all hover:shadow-lg hover:shadow-violet-500/25 disabled:opacity-50">
